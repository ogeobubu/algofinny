// server/src/controllers/bankStatement.ts - Fixed version
...
export const handleFileUpload = async (req: Request, res: Response): Promise<Response | void> => {
  try {
    ...
    form.parse(req, async (err, fields, files) => {
      ...
      try {
        file = files.statement?.[0] as File | undefined
        if (!file) {
        ...
        if (originalFilename.toLowerCase().endsWith(".json")) {
          try {
            ...
            parsedData = JSON.parse(jsonContent) as ParsedStatement
...
async function parseCSVContent(csvContent: string, filename: string): Promise<ParsedStatement> {
...
async function buildDuplicateQuery(userId: string, txn: any, bankType?: string) {
...
function createSampleJSONTemplate(bankType: string = "opay") {
  if (bankType === "opay") {
    return {
      bankType: "opay",
      accountInfo: {
